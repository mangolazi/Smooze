...
// stupidass hack to get filepath directly with an index!
QVariant QDeclarativeFolderListModel::getFilePath(int index) const
{
    QVariant rv;
    if (index != -1) {
        QModelIndex idx = d->model.index(index, 0, d->folderIndex);
        if (idx.isValid())
            rv = QUrl::fromLocalFile(d->model.data(idx, QDirModel::FilePathRole).toString());
    }
    return rv;
}
